  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      const transfers = (await db.getAllAsync(
        "SELECT * FROM transfers ORDER BY date DESC, id DESC",
      )) as Transfer[];
      ...
      const allTransactions = [
        ...records.map((record) => {
          const account = accounts.find(
            (acc) => acc.id.toString() === record.account,
          );
          return {
            fecha: record.date,
        ...
        ...transfers.map((transfer) => {
          const originAccount = accounts.find(
            (acc) => acc.id.toString() === transfer.origin,
          );
          const destinationAccount = accounts.find(
            (acc) => acc.id.toString() === transfer.destination,
          );
          return {
            fecha: transfer.date,
            tipo: "Transferencia",
      ...
      // Convertir a CSV
      const csvHeaders =
        "fecha,tipo,monto,descripcion,cuenta,cuenta_origen,cuenta_destino,id_original\n";
      const csvContent = allTransactions
        .map(
          (transaction) =>
    ...
    try {
      ...
      const summaryData = [
        ...
        { Concepto: "Total Ingresos", Valor: totalIncome },
        {
          ...
          Valor: records.filter((r) => r.type === "income").length,
        },
        { Concepto: "", Valor: "" }, // Fila vacía
        { Concepto: "Total Gastos", Valor: totalExpense },
        {
          Concepto: "Cantidad de Gastos",
          Valor: records.filter((r) => r.type === "expense").length,
        },
        { Concepto: "", Valor: "" }, // Fila vacía
        ...
        {
          Concepto: "Hora de exportación",
          Valor: new Date().toLocaleTimeString(),
        },
      ];
